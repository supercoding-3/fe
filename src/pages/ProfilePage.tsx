...
const ProfilePage: React.FC = () => {
  ...
  const [profileInfo, setProfileInfo] = useState<UserState['userInfo'] | null>(
    userInfo || null
  );
  ...
  const filteredData: ItemData[] =
    activeTab === '구매' ? purchaseData : salesData;
  ...
  return (
    <div className="profile-page">
      <div className="profile-info">
        <img
          src={profileInfo?.image || profilePlaceholder}
          alt="profile"
          ...
          {filteredData.length > 0 ? (
            filteredData.map((item) => (
              <div key={item.id} className="list-item">
                <img
                  src={item.image || profilePlaceholder}
                  alt="item"
